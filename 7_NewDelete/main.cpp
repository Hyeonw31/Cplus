...
int main()
{
	...
	//Heap는 프로그래머에 의해 할당(new)또는 소멸(delete)된다.
	//프로그래머가 원할 때 원하는 크기로 할당할 수 있다.
	//참고로 포인터 변수는 stack에 할당된다.


	int* pM = (int*)malloc(4); // C에서 동적 할당하는 기능(4. Heap 영역)
	free(pM); // 동적 메모리 할당은 반드시 제거해야한다. ( 제거 안하면 메모리 할당으로 인해 작업 느려짐 )

	// new & delete
	// new 연산자로 생성시 반드시 delete 연산자로 해제해주어야 한다
	// new 연산자로 배열 생성시 delete[] 연산자로 배열임을 명시 해주어야 한다.

	int* pA = new int(0); // C++에서 동적 할당하는 기능이 추가됨(4. Heap 영역)
	int* pArr = new int[10];
	delete pA;
	delete[] pArr;
	*/
	...
	//int* a = new int(10);
	int* b = new int(20);
	a = b;// 얕은 복사 : a의 주소값에 b의 주소값을 넣는다.-> b가 가르키고 있는 것을 a도 가리키게 함
